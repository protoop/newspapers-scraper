...
class Scraper:
    ...
    def __init__(self):
        self.name = 'Libération'
        self.homepage_url = "http://www.liberation.fr"
        self.watched_urls = []
    ...
    def start_scrapping_articles_found_on_homepage(self):
        ...
        [r.extract() for r in soup.select("#home-heading-suscriber")]
        [r.extract() for r in soup.select("a.diamond")]
        # Remove blogs
        [link.extract() for link in soup.select("a") if (link.has_attr('href') and 'blogs.' in link['href'])]

        # Articles
        most_articles = soup.select(".main-content section article a")
        lasts_articles = soup.select(".main-content section .flow .last-minute a")

        articles = most_articles + lasts_articles

        # Remove wrong urls
        [r.extract() for r in articles if r.has_attr('href') and not (r['href'].startswith('/') or self.homepage_url in r['href'])]

        articles_urls = list(set([article['href'] for article in articles if article.has_attr('href')])) #list(set(..)) for uniq. urls
    ...
    def get_article_infos_and_log_into_DB(self,url):
        ...
        try:
            if len(soup_article.find_all("meta", attrs={"property":"article:published_time"})) > 0:
                article_creation_time = soup_article.find_all("meta", attrs={"property":"article:published_time"})[0]['content']
            else:
                article_creation_time = soup_article.select("time", attrs={"itemprop":"datePublished"})[0]['datetime']
        except:
            article_creation_time = ''
        try:
            if len(soup_article.find_all("meta", attrs={"property":"article:published_time"})) > 0:
                article_update_time = soup_article.find_all("meta", attrs={"article:modified_time"})[0]['datetime']
            else:
                article_update_time = soup_article.select("time.updated")[0]['datetime']
        ...
        article_themes  = [link.text.strip() for link in soup_article.select(".breadcrumbs a") if 'Accueil' != link.text.strip()]
        article_themes += [link.text.strip() for link in soup_article.select("#breadcrumbs a") if 'Accueil' != link.text.strip()]
